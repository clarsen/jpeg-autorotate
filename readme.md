...
If the error occurs after the input file has been read, the `error` object also holds a `buffer` property containing the input file,
so it can be chained if the developer wants to ignore the error (`jo.errors.correct_orientation`, for instance) (see [pull#1](https://github.com/johansatge/jpeg-autorotate/pull/1))
...
jo.rotate('/image.jpg')
  .catch((error) => {
    if (error && error.code === jo.errors.correct_orientation) {
      ...
      console.log(error.buffer) // Holds a <Buffer>
    }
  })
```
